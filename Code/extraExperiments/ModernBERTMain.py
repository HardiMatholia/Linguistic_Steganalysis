...
# Enable TensorFloat32 precision for matrix multiplications
...
class ModernBERTTextClassifier(nn.Module):
    def __init__(self, bert_model, tfidf_dim, encoding_dim, num_classes):
...
bert_model = AutoModelForSequenceClassification.from_pretrained('answerdotai/ModernBERT-base').to(device)  # Use model with pooling
model = ModernBERTTextClassifier(bert_model=bert_model, tfidf_dim=tfidf_dim, encoding_dim=encoding_dim, num_classes=2).to(device)
